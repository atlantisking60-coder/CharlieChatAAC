# SIGN AREA - Board Hierarchy and Tab Order

> **Source of Truth** - This document defines the exact board hierarchy and tab
> ordering for the Sign area. It must only be edited manually by the project
> owner. Do not generate or modify this file programmatically.

## Tab Order (All Boards)

| #  | Board Name                              |
|----|-----------------------------------------|
| 1  | Sign Main                               |
| 2  | A-Z Of Sign                             |
| 3  | Manners and Greetings                     |
| 4  | Family and People                         |
| 5  | Feelings and Health                       |
| 6  | Questions                               |
| 7  | Grammatical Elements                    |
| 8  | Prepositions                            |
| 9  | Descriptions and Attributes               |
| 10 | Colours                                 |
| 11 | Numbers                                 |
| 12 | Quantity and Measurement                  |
| 13 | Time and Days                             |
| 14 | Letters                                 |
| 15 | Food and Drink                            |
| 16 | Personal Actions                        |
| 17 | Shared Activities                       |
| 18 | Personal Hygiene                        |
| 19 | Clothing and Personal                     |
| 20 | Personal Possessions                    |
| 21 | Home and Household                        |
| 22 | General Objects                         |
| 23 | Computer Items                          |
| 24 | School and Instructions                   |
| 25 | Leisure Activities and Interests           |
| 26 | Sport                                   |
| 27 | Animals and Nature                        |
| 28 | Weather                                 |
| 29 | Outside                                 |
| 30 | Places                                  |
| 31 | Transport and Vehicles                    |
| 32 | Money                                   |
| 33 | Public Notices                          |
| 34 | Other Countries                         |
| 35 | Religion and Customs                      |
| 36 | Gender and Sexuality                      |

---

## Full Hierarchy

```
Sign Main
A-Z Of Sign
    A (Sign)
    B (Sign)
    C (Sign)
    D (Sign)
    E (Sign)
    F (Sign)
    G (Sign)
    H (Sign)
    I (Sign)
    J (Sign)
    K (Sign)
    L (Sign)
    M (Sign)
    N (Sign)
    O (Sign)
    P (Sign)
    Q (Sign)
    R (Sign)
    S (Sign)
    T (Sign)
    U (Sign)
    V (Sign)
    W (Sign)
    X (Sign)
    Y (Sign)
    Z (Sign)
Manners and Greetings
Family and People
Feelings and Health
Questions
Grammatical Elements
Prepositions
Descriptions and Attributes
Colours
Numbers
Quantity and Measurement
Time and Days
Letters
Food and Drink
Personal Actions
Shared Activities
Personal Hygiene
Clothing and Personal
Personal Possessions
Home and Household
General Objects
Computer Items
School and Instructions
Leisure Activities and Interests
Sport
Animals and Nature
Weather
Outside
Places
Transport and Vehicles
Money
Public Notices
Other Countries
Religion and Customs
Gender and Sexuality
```

---

## Notes

- `A-Z Of Sign` contains 26 child boards (one per letter).
- `Sign Main` is the landing/home board for this area, listed first.
- All other boards are flat (no children) as they contain BSL sign images.
